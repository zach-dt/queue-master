    ...
    stage('Deploy to staging') {
      ...
      steps {
        withCredentials([usernamePassword(credentialsId: 'git-credentials-acm', passwordVariable: 'GIT_PASSWORD', usernameVariable: 'GIT_USERNAME')]) {
            ...
            sh "cd k8s-deploy-staging/ && sed -i 's#image: .*#image: ${env.TAG_STAGING}#' queue-master.yml && cat queue-master.yml"
            sh "cd k8s-deploy-staging/ && git add queue-master.yml && git commit -m 'Update queue-master version ${env.VERSION}'"
